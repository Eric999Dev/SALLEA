...
if(isset($_GET['action']) && $_GET['action'] == 'deconnexion') {
	// si l'internaute demande une déconnexion :
...
if (userConnect()) {
	// s'il est connecté, nous le redirigeons vers son profil :
...
if ($_POST) {  // équivalent à (!empty($_POST)), car si il est vide $_POST vaut implicitement false, si il est rempli il vaut implicitement true
	...
	if (empty($contenu)) {  // si vide c'est qu'il n'y a pas d'erreur sur le formulaire : on peut donc vérifier le couple login/mdp en BDD :
		...
		"SELECT * FROM membre WHERE pseudo = :pseudo AND mdp = :mdp", 
		array(':pseudo' => $_POST['pseudo'], 
			  ':mdp' => $mdp
		));
		
		if ($resultat->rowCount() != 0) {  // si il y 1 ligne dans $resultat, c'est que le login et le mdp existent et correspondent au même membre en BDD
			
			$membre = $resultat->fetch(PDO::FETCH_ASSOC);  // pas de boucle while car nous n'avons qu'un seul membre avec ce login et ce mdp
			...
			// si le nombre de ligne est de zéro, alors il n'y a pas de correspondance entre le login et le mdp 
